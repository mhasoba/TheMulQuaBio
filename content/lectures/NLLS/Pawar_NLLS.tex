...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title[Least-squares model fitting]{\bf Fitting Mathematical Models to Biological Data using Non-Linear Least-Squares Minimization (NLLS)}
% \subtitle{ }
...
\begin{itemize}[<+->]\itemsep10pt
	...
	\end{itemize}
	\pause NO: at least one parameter ($\beta$) is non-linear (e.g. $x_i^{\beta_2}$, $e^{\beta_2 x_i}$, etc.) 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Why is intrinsic non-linearity a problem for mdoel fitting?}
...
\begin{frame}{The Least-squares solution}
	...
	\begin{itemize}[<+->] \itemsep10pt
		...
		\item Let's picture this using a simple (OLS) example; fitting the model $y_1  = \beta_1 x_1  + {\color{red} \varepsilon_1 }$...
...
\begin{columns}[T]
	...
	\column{0.5\textwidth}
		\begin{align*}
		  y_1  & = \beta_0 + \beta_1 x_1  + {\color{red} \varepsilon_1 }\\
		  \\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{What NLLS solves (and how)}

\begin{itemize}[<+->]\itemsep10pt

	\item In an {intrinsically non-linear model} such as 
	$$y_i = \beta_0 e^{\beta_2 x_i} + \varepsilon_i ~,$$ 	
	the nice trick of solving  $\mathbf{Y} = \mathbf{X \boldsymbol\beta  +
	\boldsymbol\varepsilon}$ algebraically is impossible
	
	\item But we can use brute-force computation to find close-to-optimal least squares minimization:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{The NLLS optimization process}
...
\begin{itemize}[<+->]\itemsep10pt
	...
	\item The {\bf Gauss-Newton} algorithm is the default in the {\tt nls} 
	...
	\item The {\bf Levenberg-Marquardt (LM)} algorithm switches switches between
	Gauss-Newton and ``gradient descent'' and is more robust against starting
	values that are far-off-optimal 
	
	\item the LM algorithm is available in {\tt R} through the the {\tt
	minpack.lm} package {\small \url
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Practical: Michaelis-Menten biochemical kinetics}
...
\begin{itemize}\itemsep10pt
	...
	\item You will use NLLS fitting to obtain estimates of $V_{\max}$ and $K_H$

	\item Note that $V_{\max} \le 0 $ and $K_H \le 0$ are physically impossible (useful fir picking starting values)

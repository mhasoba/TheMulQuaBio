...
\item Linear and non-linear least squares model fitting:  (and mathematically/algorithmically simple) approaches, useful in many scenarios in biology 
\begin{itemize}
	\item {\it Non-linear} Least Squares (NLLS) fitting  is often necessary because many mechanisms in biology are inherently non-linear (i.e., r data are better-explained by a non-linear mathematical model)
\end{itemize} 

\item MLE/Bayesian methods: Versatile and powerful more robust if you are able to calculate the likelihood function analytically or numerically 

...
\title[NLLS]{\bf Fitting models to data using Non-linear Least-Squares}
...
\begin{itemize}\itemsep16pt
	...
	\item NLLS in {\tt R}
	\item Afternoon practicals overview (two examples)
\end{itemize}
...
\begin{itemize}[<+->]
	\pause 
	\item These are {\it all} good linear models (huh?!)
	\item The data can be modelled as {\it linear combination} of 
	variables and coefficients
	\item Easily fitted using Ordinary Least Squares (OLS) regression
	\item Linear models can include curved relationships (e.g. polynomials)
	\item {\it OK, so then why Non-Linear Least Squares (NLLS) fitting?} 
\end{itemize}
...
% \begin{itemize}[<+->]\itemsep10pt
	...
	% \item OLS regression can be used to fit both linear and nonlinear 
	% {\it equations} that {\it intrinsically linear}\\
	% \pause e.g., Straight line: $y_i = \beta_0 + \beta_1 x_i + \varepsilon_i$\\
	% \pause e.g., Polynomial: $y_i = \exp(\beta_0) + \beta_1 x_i + \beta_2 x_i^2 
	% + \varepsilon_i$

	% \item Indeed, for OLS to work, we need {\it intrinsic linearity} --- 
	% i.e., the equation to be fitted (model) should be {\it linear in the 
	% parameters}\\
	
	% \item Are these models linear in their parameters?  
	% \begin{itemize}
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \begin{frame}{The linear least-squares solution}

% OLS minimizes the {\it sum} of the {\it squared} residuals

%% animate directly from a multipage pdf
%% use timeline to only embed the axes once!
% \begin{columns}[T]
% \column{\textwidth}
...
% There is only one unique solution, which gives the OLS fitted 
	% parameter values for $\boldsymbol\beta = (\beta_0, \beta_1)$:
...
\begin{itemize}[<+->]\itemsep10pt
	\item Choose initial values for the $\beta_j$'s
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \begin{frame}{Enter NLLS!}

% The end result is (approximately) the same:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{OK, fine, why would {\it I} ever need to worry about NLLS?}

\begin{itemize}[<+->]\itemsep10pt
	\pause 
	\item Many observations in biology are just not well-fitted by a linear model

	\item That is, the underlying biological phenomena are not 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Example 1: Temperature and metabolism}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Example 2: Functional responses}
...
the data points} (step 4) has at least two algorithms:
...
NLLS-regression has all the assumptions of OLS-regression:
	\begin{itemize}[<+->]\itemsep10pt
		...
		instead! 
		(e.g., using {\tt nlm} for optimizing/fitting) 
...
\begin{itemize}
	...
	\item That is, the likelihood of a set of parameter values (of a 
	model), $\theta$, given outcomes $x$, equals the probability of those 
...
The easiest thing to do for you is to use information theory (including AIC and BIC) to compare 
models. \\

{\it Both use estimated likelihood of a model}\\

\pause This is how you can calculate these (using {\tt R} syntax): 

\pause
\begin{itemize}
\small
	\item residuals = Observations - Predictions
	\item {\tt rss = sum(residuals \^{} 2)} 
	
	\item Then, AIC is {\tt n * log((2 * pi) / n) + n + 2 + n * log(rss) + 
	2 * k} \\
		({\it what is {\tt n} and {\tt k}?})
	
	\item And BIC is {\tt n + n * log(2 * pi) + n * log(rss / n) + 
	(log(n)) * (k + 1)}
...
{\tt tss = sum((Observations - mean(Predictions)) \^{} 2)}
\end{itemize}

\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Practicals: Instructions}

\begin{itemize}\itemsep12pt
	
	\item We shall start off with some simple examples 
	({\tt NLSFitEx1.R}, {\tt NLSFitEx2.R}, etc.)

	% \item You can then choose either Practical 1 or 2, or do both!

	\item In each, make sure you have a good look at the data first by plotting 
	them up in a loop.
	
	\item Keep workflow organized in {\tt Code}, {\tt Results}, {\tt Data} ! 
	
	\item You may also find your own model to fit some data related to your interests 
	or project

	% \item Your demonstrators and I will help you get started in any case
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Practicals: Basic examples}

\begin{itemize}\itemsep12pt
	
	\item Go to \url{mhasoba.bitbucket.org} and click on the sailfin. (This takes you to \url{https://bitbucket.org/mhasoba/silbiocompmasterepo/src/92f5d9b6910f?at=master})
	
	\item Create {\tt Code}, {\tt Results}, {\tt Data} directories on your own computer 

	\item Download {\tt NLSFitEx1.R} and {\tt NLSFitEx2.R} from the {\tt Code} directory, and the associated data from the {\tt Data} directory.

	\item Run the both scripts and examine them line by line  

	\item Next you can try Practical 1 or 2 (Next two slides)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \begin{frame}{Practical 1: Fitting thermal responses}

% \begin{itemize}
	
	% \item Use {\tt nls} (or {\tt nlsLM}) to fit the {\tt ThermRespData.csv} 
	% dataset to the model: $B = B_0 \boxed {e^{-\frac{E}{kT}}}f(T,T_{pk},E_D)$\\

  % \item Plot the data and output coefficient estimates and fit stats to 
  % a file 
  
  % \item Here's an example of how a fitted curve looks (only two 
  % fitted parameters are shown): \\

	% \centering 
	% \includegraphics[width=.45\textwidth]{164.pdf}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Practical: Fitting Functional responses}

\begin{itemize}
	
	\item Use {\tt nls} (or {\tt nlsLM}) to fit {\tt CRat.csv} 
	dataset to the model: $f(x_R) = c = \frac{{a x_R^{q + 1}}}{{1 + h a x_R^{q + 1}}}$\\
	($c$ is consumption rate)
  \item Plot the data and output coefficient estimates and fit stats to 
  a file 
  
  \item Here's an example of how a fitted curve looks : \\
 
\end{itemize}

	\centering 
	\includegraphics[width=.4\textwidth]{FR_eg.pdf}
